...
class Gateway extends \ParadoxLabs\TokenBase\Model\AbstractGateway
{
    ...
    protected function runTransaction($request, $params)
    {
        ...
        /*@start Added by Ramakanata - 25th Apr2024 - to debug*/
        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/debug.log');
        $logger = new \Zend_Log();
        $logger->addWriter($writer);
        $logger->info(__METHOD__);
        $requestArr = $this->lastRequest;
        $logger->info('Refund ENDPOINT : '.$this->endpoint);
        $logger->info('Refund requestArr ='.print_r($requestArr,true));
        /*@end Added by Ramakanata - 25th Apr2024 - to debug*/
        ...
        /*@start Added by Ramakanata - 25th Apr2024 - to debug*/
        $responseArr = $this->lastResponse;
        $logger->info('Refund responseArr ='.print_r($responseArr,true));
        /*@end Added by Ramakanata - 25th Apr2024 - to debug*/
    ...
    protected function interpretTransaction($transactionResult)
    {
        ...
        if ($transactionResult['messages']['resultCode'] !== 'Ok'
            ...
            if ((int)$response->getResponseReasonCode() === 65) {
				throw new CommandException(
					__('CVV is incorrect.')
				);
			}elseif ((int)$response->getResponseReasonCode() === 27) {
				throw new CommandException(
					__('billing ZIP code does not match.')
				);
			} else {
				throw new CommandException(
