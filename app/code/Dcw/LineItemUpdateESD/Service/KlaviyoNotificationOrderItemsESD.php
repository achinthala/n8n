<?php
namespace Dcw\LineItemUpdateESD\Service;
...
class KlaviyoNotificationOrderItemsESD
{
    /**
    ...
    /**
     * Get custom logger instance for OrderItemsESDUpdate.log
     *
     * @return \Zend_Log
     */
    public function getCustomLogger()
    {
        if ($this->customLogger === null) {
            $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/OrderItemsESDUpdate.log');
    ...
    /**
     * Trigger OrderItemsESD notification to Klaviyo
     *
    ...
    public function triggerOrderItemsESDNotificationToKlaviyo(OrderItem $orderLineItem, $estimatedShipDate)
    {
        ...
        if (empty($customerEmail)) {
            $this->getCustomLogger()->warn('OrderItemsESDNotification: No customer email, cannot send to Klaviyo - Order ID: ' . $orderId . ' | Item ID: ' . $orderLineItem->getId());
            return;
        }
            ...
            try {
                ...
                if ($parentProductId) {
                    try {
                        ...
                        $this->getCustomLogger()->warn('OrderItemsESDNotification: Failed to load parent product from repository - Order ID: ' . $orderId . ' | Item ID: ' . $orderLineItem->getId() . ' | Parent Product ID: ' . $parentProductId . ' | Error: ' . $e->getMessage());
                    }
                }
            } catch (\Exception $e) {
                $this->getCustomLogger()->warn('OrderItemsESDNotification: Error getting parent product info - Order ID: ' . $orderId . ' | Item ID: ' . $orderLineItem->getId() . ' | Error: ' . $e->getMessage());
            }
        }
        ...
        elseif ($currentProductType === 'configurable' && $currentProductId) {
            ...
            try {
                ...
                $this->getCustomLogger()->warn('OrderItemsESDNotification: Failed to load configurable product - Order ID: ' . $orderId . ' | Item ID: ' . $orderLineItem->getId() . ' | Parent Product ID: ' . $parentProductId . ' | Error: ' . $e->getMessage());
            }
        }
                ...
                if ($currentProductId) {
                    ...
                    if (!empty($parentIds) && isset($parentIds[0])) {
                        ...
                        try {
                            ...
                            $this->getCustomLogger()->warn('OrderItemsESDNotification: Failed to load parent product via configurable lookup - Order ID: ' . $orderId . ' | Item ID: ' . $orderLineItem->getId() . ' | Parent Product ID: ' . $parentProductId . ' | Error: ' . $e->getMessage());
                        }
                    }
                }
            } catch (\Exception $e) {
                $this->getCustomLogger()->warn('OrderItemsESDNotification: Error checking configurable parent relationship - Order ID: ' . $orderId . ' | Item ID: ' . $orderLineItem->getId() . ' | Error: ' . $e->getMessage());
            }
        }
        ...
        if (!$productForUrl) {
            try {
                ...
                $this->getCustomLogger()->warn('OrderItemsESDNotification: Failed to load current item product - Order ID: ' . $orderId . ' | Item ID: ' . $orderLineItem->getId() . ' | Error: ' . $e->getMessage());
            }
        }
        ...
        if ($isKlaviyoEnabled && $privateApiKey) {
                    ...
                    'attributes' => [
                        ...
                        'metric' => [
                            'data' => [
                                'type' => 'metric',
                                'attributes' => [
                                    'name' => 'EstimatedShipDateUpdateForLineItems',
                                    'service' => 'magentotwo'
                                ]
                            ]
                        ],
            ...
            $this->getCustomLogger()->info(json_encode([
            'event'    => 'OrderItemsESDNotification',
            'status'   => 'eventData',
            'order_id' => $orderId,
            'item_id'  => $orderLineItem->getId(),
            'payload'  => $eventData
            ]));
            ...
            if ($httpCode == 202 || $httpCode == 200) {
                $this->getCustomLogger()->info(
                    'OrderItemsESDNotification: Klaviyo API call successful - Order ID: '
                    . $orderId . ' | Item ID: ' . $orderLineItem->getId()
                    . ' | estimatedShipDate: ' . $estimatedShipDate . ' | HTTP Code: ' . $httpCode
                );

                // Mark this order item as notified about ESD delay.
                try {
                    $orderLineItem->setData('esd_delay_notified', 0);
                    $this->orderItemResource->save($orderLineItem);
                } catch (\Exception $e) {
                    $this->getCustomLogger()->warn(
                        'OrderItemsESDNotification: Failed to update esd_delay_notified flag - Order ID: '
                        . $orderId . ' | Item ID: ' . $orderLineItem->getId()
                        . ' | Error: ' . $e->getMessage()
                    );
                }
            } else {
                $this->getCustomLogger()->err('OrderItemsESDNotification: Klaviyo API Error - HTTP Code: ' . $httpCode . ' | Response: ' . $response . ' | CURL Error: ' . $curlError . ' | Order ID: ' . $orderId . ' | Item ID: ' . $orderLineItem->getId() . ' | Customer Email: ' . $customerEmail . ' | estimatedShipDate: ' . $estimatedShipDate);
            }
        } else {
            $this->getCustomLogger()->warn('OrderItemsESDNotification: Klaviyo not enabled or no API key - Order ID: ' . $orderId . ' | Item ID: ' . $orderLineItem->getId() . ' | Klaviyo Enabled: ' . ($isKlaviyoEnabled ? 'Yes' : 'No') . ' | Has API Key: ' . (!empty($privateApiKey) ? 'Yes' : 'No'));
        }
    }
}
