        ...
        if (isset($data['product']['media_gallery']['images'])) {
            ...
            if (isset($mediaGallery['images'])) {
                foreach ($mediaGallery['images'] as $image) {
                    ...
                    // CSV-imported rows may not post custom_image_link; do not clear them.
                    if (!array_key_exists('custom_image_link', $image)) {
                        continue;
                    }

                    $vmValue = !empty($image['vm']) ? (int) $image['vm'] : 0;
                    $customImageLinkValue = trim((string) $image['custom_image_link']);
