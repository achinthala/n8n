...
namespace Dcw\OrderPendingReview\Plugin\Ui;

use Magento\Framework\App\ResourceConnection;
use Magento\Framework\View\Element\UiComponent\DataProvider\DataProvider;

/**
 * Adds dcw_pending_review_reasons from sales_order to the admin order grid payload.
 */
class SalesOrderGridDataProviderPlugin
{
    ...
    public function afterGetData(DataProvider $subject, array $data): array
    {
        ...
        $select = $connection->select()
            ->from($orderTable, ['entity_id', 'dcw_pending_review_reasons'])
            ->where('entity_id IN (?)', $ids);

        $map = [];
        foreach ($connection->fetchAll($select) as $row) {
            $map[(int) $row['entity_id']] = $row['dcw_pending_review_reasons'];
        }
        ...
        foreach ($data['items'] as &$item) {
            ...
            if ($id > 0 && isset($map[$id])) {
                $item['dcw_pending_review_reasons'] = $map[$id];
            }
        }
        unset($item);
