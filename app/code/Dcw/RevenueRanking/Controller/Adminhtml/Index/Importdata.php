...
class Importdata extends Action
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            $filePath = BP . '/pub/media/import/sql.csv';
            ...
            // Process rows
            [$updatedCount, $errors, $productsToUpdate] = $this->processCsvRows(
                $csvData,
                $skuIndex,
            ...
            $attributeUpdatedCount = $this->updateProductAttributes($productsToUpdate);

            return $this->prepareFinalResult($resultJson, $updatedCount, $attributeUpdatedCount, $errors);
        } catch (\Exception $e) {
            $this->logger->error($e->getMessage());
            return $this->jsonError($resultJson, $e->getMessage());
        }
    }
    ...
    private function validateCsvHeaders(array $headers): array
    {
        ...
        foreach ($headers as $header) {
            ...
            $header = preg_replace('/\x{FEFF}/u', '', $header); // remove BOM if present
            $normalized[] = $header;
        }
        $headers = array_map('strtolower', array_map('trim', $normalized));
        $skuIndex = array_search('sku', $headers);
        $manualAdjustmentIndex = array_search('manual_adjustment', $normalized);
    ...
    private function processRow(array $row, int $rowIndex, int $skuIndex, int $manualAdjustmentIndex): array
    {
        ...
        foreach ($collection as $item) {
            ...
            $baseRevenue = $adjustmentValue;
            $adjustedRevenue = $baseRevenue;

            $item->setManualAdjustment(0)
                ->setAdjustedRevenue($adjustedRevenue)
                ->setBaseRevenue($adjustedRevenue)
                ->setSku($sku)
                ->save();
