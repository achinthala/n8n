...
define([
...
], function (totals, sdk, jQuery, quote, getTotalsAction, fullScreenLoader, storage, setShippingAddress, certificateAuthenticationPopup, customerDataModel, customerModel, stepNavigator) {
    'use strict';
    ...
    return function (targetModule) {
        ...
        targetModule.prototype.initialize = function initialize() {
            ...
            //this.refreshCache(); console.log("on page init hello");
            ...
            jQuery(window.document).on('checkout.navigateTo', function () {
                this.hasUploadedCertificate(false); console.log("new path 3");
                this.refreshCache();
            }.bind(this));

            //call on payment tab 
            stepNavigator.steps.subscribe(function (steps) {
                var paymentStep = steps.find(function (step) {
                    return step.code === 'payment';
                });

                if (paymentStep && paymentStep.isVisible()) {
                    var taxTotal = totals.getSegment('tax');
                    if(taxTotal==0){ console.log("tax_total_zero");
